...
{
	ExpectTokenInformations("*", { TokenInformation(Token::MULTIPLY, StreamString("*", StreamPosition())) });
}

TEST(multiply_token, determining_if_stay_between_delimiters)
{
	ExpectTokenInformations(" * ", { TokenInformation(Token::MULTIPLY, StreamString("*", StreamPosition(1, 2))) });
	ExpectTokenInformations(
		";*;",
		{ TokenInformation(Token::SEMICOLON, StreamString(";", StreamPosition())),
		  TokenInformation(Token::MULTIPLY, StreamString("*", StreamPosition(1, 2))),
...
TEST(multiply_token, determining_if_stay_near_delimiter)
{
	ExpectTokenInformations(
		"*;",
		{ TokenInformation(Token::MULTIPLY, StreamString("*", StreamPosition())),
...
TEST(multiply_token, determining_if_stay_between_numbers)
{
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::FLOAT, StreamString("1.1", StreamPosition())),
		  TokenInformation(Token::MULTIPLY, StreamString("*", StreamPosition(1, 4))),
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::EXPONENTIAL, StreamString("1_E+1", StreamPosition())),
		  TokenInformation(Token::MULTIPLY, StreamString("*", StreamPosition(1, 6))),
...
TEST(multiply_token, not_determining_if_part_of_plus_assignment)
{
	...
	ExpectTokenInformations(
		"*=*",
		{ TokenInformation(Token::MULTIPLY_ASSIGNMENT, StreamString("*=", StreamPosition())),
		  TokenInformation(Token::MULTIPLY, StreamString("*", StreamPosition(1, 3))) });
}
