...
{
	ExpectTokenInformations(">", { TokenInformation(Token::MORE, StreamString(">", StreamPosition())) });
}

TEST(more_token, determining_if_stay_between_delimiters)
{
	ExpectTokenInformations(" > ", { TokenInformation(Token::MORE, StreamString(">", StreamPosition(1, 2))) });
	ExpectTokenInformations(
		";>;",
		{ TokenInformation(Token::SEMICOLON, StreamString(";", StreamPosition())),
		  TokenInformation(Token::MORE, StreamString(">", StreamPosition(1, 2))),
...
TEST(more_token, determining_if_stay_near_delimiter)
{
	ExpectTokenInformations(
		">;",
		{ TokenInformation(Token::MORE, StreamString(">", StreamPosition())),
...
TEST(more_token, determining_if_stay_between_numbers)
{
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::FLOAT, StreamString("1.1", StreamPosition())),
		  TokenInformation(Token::MORE, StreamString(">", StreamPosition(1, 4))),
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::EXPONENTIAL, StreamString("1_E+1", StreamPosition())),
		  TokenInformation(Token::MORE, StreamString(">", StreamPosition(1, 6))),
