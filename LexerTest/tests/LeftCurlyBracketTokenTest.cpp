...
{
	ExpectTokenInformations("{", { TokenInformation(Token::LEFT_CURLY_BRACKET, StreamString("{", StreamPosition())) });
}

TEST(left_curly_bracket_token, determining_if_stay_between_delimiters)
{
	ExpectTokenInformations(
		" { ", { TokenInformation(Token::LEFT_CURLY_BRACKET, StreamString("{", StreamPosition(1, 2))) });
	ExpectTokenInformations(
		";{;",
...
TEST(left_curly_bracket_token, determining_if_stay_near_delimiter)
{
	ExpectTokenInformations(
		"{;",
		{ TokenInformation(Token::LEFT_CURLY_BRACKET, StreamString("{", StreamPosition())),
...
TEST(left_curly_bracket_token, not_determining_if_stay_between_numbers)
{
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::FLOAT, StreamString("1.1", StreamPosition())),
		  TokenInformation(Token::LEFT_CURLY_BRACKET, StreamString("{", StreamPosition(1, 4))),
	...
	ExpectTokenInformations(
		...
		{ TokenInformation(Token::EXPONENTIAL, StreamString("1_E+1", StreamPosition())),
		  TokenInformation(Token::LEFT_CURLY_BRACKET, StreamString("{", StreamPosition(1, 6))),
